    ...
    $.fn.imageUploader = function (options) {
        ...
        let createImg = function (src, id, preloaded) {
            ...
            let $container = $('<div>', {class: 'uploaded-image'}),
                ...
                $button = $('<button>', {class: 'delete-image',fileid: id}).appendTo($container),

                // Create the delete icon
                $i = $('<i>', {class: 'lni lni-trash', title: "Remover imagem"}).appendTo($button);
            ...
            $button.on("click", function (e) {
                ...
                var confirmacao = confirm('Tem certeza que quer remover a imagem? A ação não poderá ser desfeita!');
        ...
        let validateExtension = function (file) {
            ...
            if (plugin.settings.extensions.indexOf(file.name.replace(new RegExp('^.*\\.'), '.')) < 0) {
                alert(`O arquivo "${file.name}" não possui uma extensão permitida: "${plugin.settings.extensions.join('", "')}"`);
        ...
        let validateMIME = function (file) {
            ...
            if (plugin.settings.mimes.indexOf(file.type) < 0) {
                alert(`O arquivo "${file.name}" não é permitido: "${plugin.settings.mimes.join('", "')}"`);
        ...
        let validateMaxSize = function (file) {
            ...
            if (file.size > plugin.settings.maxSize) {
                alert(`O arquivo "${file.name}" excede o tamanho máximo ${plugin.settings.maxSize / 1024 / 1024}Mb`);
        ...
        let validateMaxFiles = function (index, file) {
            ...
            if ((index + dataTransfer.items.length + plugin.settings.preloaded.length) >= plugin.settings.maxFiles) {
                alert(`O arquivo "${file.name}" não pode ser adicionado pois ultrapassou o limite de ${plugin.settings.maxFiles} arquivos`);
